...
def get_3_prompt(code_snippet, warning_type):
    prompt = f"""Please analyze whether the following code has a {warning_type} vulnerability:
    Answer in YES or NO.
...
def process_case(code_snippet, warning_type):
    # 这部分对话记录
    messages = []
    # 结果
    result = {}
    ...
    # 正常检测的prompt
    prompt = get_3_prompt(code_snippet, warning_type)
    # 大模型判断：该代码是否符合规则
...
if __name__ == "__main__":
    ...
    for run_type in run_type_list:
        ...
        output_file = f"{date}/output-vul-type/{run_type}/{date}-{run_type}-output-vul-type.json"
        ...
        with open(input_file, 'r', encoding='utf-8') as file:
            ...
            for item in items:
                ...
                result = process_case(code_snippet, warning_type)
