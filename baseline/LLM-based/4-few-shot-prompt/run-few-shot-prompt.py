...
def get_few_shot_prompt(code_snippet, warning_type, example):
    prompt = f"""This is an example of a {warning_type} vulnerability:
    ### example:{example}
    Please analyze whether the following code has a similar {warning_type} vulnerability:
    Answer in YES or NO.
...
def process_case(code_snippet, warning_type, example):
    # 这部分对话记录
    messages = []
    # 结果
    result = {}
    ...
    # 正常检测的prompt
    prompt = get_few_shot_prompt(code_snippet, warning_type, example)
    # 大模型判断：该代码是否符合规则
    ...
    if response in ("Yes", "YES", "yes"):  # 大模型返回YES：real bug
        ...
        while (response not in ["Yes", "YES", "yes", "No", "NO", "no"]) and (try_count < 3):
            ...
            summary_response = call_model(messages)
...
if __name__ == "__main__":
    ...
    for run_type in run_type_list:
        ...
        warning_type_dict = {
            "DivideByZero-covers": "DivideByZero",
            "DivideByZero-not-covers": "DivideByZero",
            "DivideByZero-real-bug": "DivideByZero",
            "Nullpointer": "Nullpointer",
            "OutOfBound-covers": "OutOfBound",
            "OutOfBound-not-covers": "OutOfBound",
            "OutOfBound-real-bug": "OutOfBound"
        }
        warning_type = warning_type_dict[run_type]

        example_dict = {
            "DivideByZero": """
            void calculateDivision() {
                int numerator = 10;
                int denominator = 0;  // 分母为零
                int result = numerator / denominator;  // 尝试除以零
            ...
            void displayValue() {
                ...
                std::cout << "Value: " << *ptr << std::endl;  // 尝试空指针解引用
            }
            """,
            "OutOfBound": """
            void accessArrayElement() {
                int arr[5] = { 0, 1, 2, 3, 4 };
                int index = 5;  // 无效索引，超出数组范围
                std::cout << "Element: " << arr[index] << std::endl;  // 尝试访问无效索引
            }
            """
        }
        example = example_dict[warning_type]
        ...
        output_file = f"{date}/{date}-{run_type}-output-few-shot-prompt.json"
        ...
        with open(input_file, 'r', encoding='utf-8') as file:
            ...
            for item in items:
                ...
                result = process_case(code_snippet, warning_type, example)
